...
def dep_list(current_semester):
	...
	"""
	courses = (s.query(Course.major_code).filter_by(acad_period=current_semester).distinct(Course.major_code).
	order_by(Course.major_code).all())
	...
	for i in courses:
		if str(i[0]) != "":
			deps.append(str(i[0]))
	wrap_num = int(math.ceil(len(deps)/3.0))
	for j in range(wrap_num):
		final.append(deps[j])
		try:
			final.append(deps[j+wrap_num])
			final.append(deps[j+(wrap_num*2)])
		except:
			pass
...
def read_notes_file():
	...
	with open('/var/www/html/Katy/static/ScheduleNotes.csv', 'rU') as f:
...
def process_search():
	...
	if request.method == "POST":
		...
		# see if there are any potential search terms that could	
		# indicate searching an acad_period			
		poss_acad_periods=[]			
		for i in query_term.split(" "):			
			if len(i)==6 and i.isdigit():			
				poss_acad_periods.append(i)
		# if no terms are potential acad_periods add			
		# the current semester to the search term
		if len(poss_acad_periods) == 0:			
			query_term +=" "+current_semester			
...
ACCESS_TOKEN_FILE = "/var/www/html/Katy/access_token.txt"
